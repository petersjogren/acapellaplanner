...
- **Parking lot — film nudges / crop reorder (do not implement unless asked).** v1 camera is uniform time-per-scrollable-pixel. Notation density ≠ duration; optional waypoints that warp `F`, and a reorder UI, are later overlays — not pins on day one.
