...
- `min-h-screen` (`100vh`) undercounts Safari's dynamic toolbar; use `min-h-dvh` for phone-viewport shells. **`min-h-dvh` is not a definite height.** A column of `flex-1` + child `h-full` / `overflow-hidden` then collapses the film to 0 px (content-sized auto, clipped). The booth shell needs `h-dvh` (plus `min-h-dvh`) so leftover space is real; pin `SheetCue` with `absolute inset-0` in a `relative flex-1 min-h-0` slot.
...
Fix: use a **callback ref** stored in `useState` (`const [el, setEl] = useState<HTMLElement|null>(null)`, `ref={setEl}`) and key the measuring `useEffect`'s deps on that state value, not `[]`. A callback ref re-fires every time React attaches it to a new/different DOM node, however many renders that takes — so the effect (and its `ResizeObserver`) attaches correctly whenever the real element finally shows up, not just on the component instance's first commit. Any component measuring a node that might not exist on first render — behind a loading state, an async fetch, or a conditional — needs a callback ref for this reason, not `useRef` + empty-deps `useEffect`. The same observer now also reads `clientHeight`: slide width is `measuredHeight × aspect` so the strip fills the dock booth. Unmeasured height falls back to `SHEET_CUE_SLIDE_HEIGHT_PX` (jsdom / first paint). Abort `setState` on unmount (`cancelled`) so a late measure cannot run after jsdom tears down `window`.
